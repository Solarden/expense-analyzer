"""myFund.pl API client — the opt-in network positions source.
...
The API returns many numbers as strings, some
...
class MyFundClient:
    ...
    def fetch(self) -> PositionsResult:
        ...
        try:
            # SECURITY: never enable follow_redirects. The API key rides in the
            # query string, so a compromised host could 30x us to an attacker and
            # httpx would replay the key-bearing URL there. Explicit, not defaulted.
            with httpx.Client(
